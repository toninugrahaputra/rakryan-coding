...
import { update } from '@/actions/App/Http/Controllers/Internal/ProductGuideController';
...
export default function GuidesEdit({ product, guide }: { product: Product; guide: GuideProp }) {
    const editorRef = useRef<EditorJsRef>(null);

    const [form, setForm] = useState({
        title: guide.title,
        slug: guide.slug,
        content: guide.content as OutputData | null,
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        router.put(update({ product: product.slug, guide: guide.slug }).url, {
            title: form.title,
            slug: form.slug,
    ...
    return (
        <>
            <Head title={`Edit — ${guide.title}`} />

            <ScrollToTopBottom />

            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div>
                    <h1 className="text-2xl font-semibold">Edit Panduan</h1>
